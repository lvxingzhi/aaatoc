# RecallAAA - 副本 / 怪物 / 技能速查插件

魔兽世界插件：`/recall` 打开，手动点击浏览副本 → 怪物 → 技能，三个窗口可拖拽移动、右下角调整大小。
...
├── package.py           # 打包脚本 (运行它生成 RecallAAA.zip)
...
1. 把 `addon` 目录复制到 `World of Warcraft/_retail_/Interface/AddOns/`，改名为 `RecallAAA`
2. 游戏内 `/reload`
3. `/recall` 打开
...
> 若游戏提示插件过期，把 `addon/RecallAAA.toc` 里的 `## Interface:` 改成当前版本号
